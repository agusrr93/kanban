<template>
    <ol class="kanban  Done" v-if="review.length!=0">
    <h2><i class="fa fa-caret-square-o-right" aria-hidden="true"></i>
 Review</h2>
          <div v-for="to in review" v-bind:key="to.id">
            <li class="dd-item">
...
<script>
    ...
    methods: {
        ...
        next: function (param) {
            var ref = db.ref(`review/${param.id}`)
            ref.set({});
            var refn = db.ref(`done/`)
            refn.push({
                title: param.title,
